    ...
    <form action="{{ route('pastes.store') }}" method="POST">
        ...
        <div class="mb-6">
            <input type="text" name="title" value="{{ old('title') }}"
                class="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-semibold text-lg text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:font-normal placeholder:text-slate-400 @error('title') border-rose-500 focus:ring-rose-500/20 @enderror"
